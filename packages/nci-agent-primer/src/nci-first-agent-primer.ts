...
const NCI_FIRST_AGENT_PRIMER_COMPACT = [
  ...
  "1. **query active-package <packageName>** — resolves the installed **package_version**, confirms **indexed=true**, and returns **packageManager** for the directory NCI was invoked from. On **`--format json`**, read **`meta.activePackageResolution`**: **`directInstallPath`** means only the hoisted `node_modules/<pkg>` (or `@scope/pkg`) paths were checked per workspace root; **`fullScanFallback`** means a full top-level scan ran because no hoisted directory matched (unusual folder layout). Pin the resolved **package_version** in all subsequent calls unless you are intentionally comparing versions.",
  ...
  "**Query JSON (`--format json`):** every successful **`query …`** response includes **`meta`** next to **`data`** (**`envelopeVersion`**, **`query`**, plus limits and truncation). Prefer **`meta`** for programmatic checks; **`data`** shapes stay stable.",
...
const NCI_FIRST_AGENT_PRIMER_LIVE = [
  ...
  "1. **`query active-package <packageName>`** — resolve the project's installed `package_version` and confirm `indexed=true`. JSON includes **`packageManager`** for **the directory NCI was invoked from** (the project context resolved at call time). It also includes **`meta.activePackageResolution`**: **`directInstallPath`** when only hoisted install paths were probed per `node_modules` root (fast), or **`fullScanFallback`** when those paths missed and a legacy top-level enumeration ran. Use the resolved `package_version` to pin step 2 (and any later `query symbol` / `query find`) unless you intentionally compare versions, or only one indexed version exists for this name.",
  ...
  '- **Evidence response shape:** the JSON envelope carries `data.symbols` (deduped hits, see truncation marker below) and `data.snippets` — a JSON **object keyed by `symbols.id`** whose values hold the same stored `signature` / `js_doc` text that `query snippet "<id>"` reads. `data.snippets[id]` is cite-ready content for any id present. **`meta`** (always present on successful `query --format json`) duplicates truncation and limit facts for structured clients.',
  ...
  '- **Truncation marker:** when the trailing element of `data.symbols` has `id`, `name`, AND `kindName` all equal to the literal string `"<truncated>"`, more results existed than `--limit` allowed. Either raise `-n` / `--limit`, narrow with `--package-version` / `--source-package` / `--kind`, or pick a more specific anchor — do **not** switch to `read` / `grep` to compensate. Real symbol rows never use that sentinel. **`--format json`:** check **`meta.truncated`** on the same envelope for the same signal without parsing sentinel rows.',
