...
describe("Environmental Context", () => {
  const fixtureDir = path.resolve(
    __dirname,
    "../fixtures/environmental-context",
  );
  ...
  it("generates synthetic IDs for environmental and custom protocol dependencies", () => {
    ...
    const handleMethod = graph.symbols.find(
      (symbol) => symbol.name === "Handler.handle",
    );
    expect(handleMethod).toBeDefined();
    expect(handleMethod?.dependencies).toContain("node::http::ServerResponse");

    const processMethod = graph.symbols.find(
      (symbol) => symbol.name === "Handler.process",
    );
    expect(processMethod).toBeDefined();
    expect(processMethod?.dependencies).toContain(
      "ext::custom-system::CustomType",
    );

    const pipeMethod = graph.symbols.find(
      (symbol) => symbol.name === "Handler.pipe",
    );
