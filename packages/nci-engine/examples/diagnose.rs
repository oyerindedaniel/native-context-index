...
use nci_engine::cache::{NCI_ENGINE_VERSION, nci_cache_base_dir, nci_sqlite_path};
...
fn main() {
    ...
    match nci_cache_base_dir() {
        Some(ref path) => {
            ...
            println!("  exists: {}", path.exists() && path.is_dir());
        }
        None => println!(
            "nci_cache_base_dir: (none — dirs::cache_dir and NCI_CACHE_DIR both unavailable)"
        ),
    }
    ...
    match nci_sqlite_path() {
        Some(ref sqlite_path) => {
            ...
            println!("  file_exists: {}", sqlite_path.exists());
            if let Some(parent) = sqlite_path.parent() {
                println!("  parent_exists: {}", parent.exists() && parent.is_dir());
            }
            ...
            match NciDatabase::open(sqlite_path) {
                ...
                Err(error) => {
                    ...
                    if sqlite_path.exists() {
                        println!(
                            "  hint: file exists but open/migrate failed; check permissions or SchemaTooNew message above"
                        );
                    } else {
                        println!(
                            "  hint: database file not created yet (first index will create it)"
                        );
                    }
                }
            }
