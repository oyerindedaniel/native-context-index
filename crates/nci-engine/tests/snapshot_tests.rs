...
use nci_engine::types::{PackageGraph, PackageInfo};
...
fn normalise_graph(graph: &mut PackageGraph, fixture_dir: &Path) {
    ...
    for symbol in &mut graph.symbols {
        ...
        if abs.starts_with(&fixture_dir_str) {
            ...
            symbol.file_path = rest.strip_prefix('/').unwrap_or(rest).to_string();
        }

        if let Some(ref mut files) = symbol.additional_files {
            for file in files.iter_mut() {
                let abs = file.replace('\\', "/");
                if abs.starts_with(&fixture_dir_str) {
                    let rest = &abs[fixture_dir_str.len()..];
                    *file = rest.strip_prefix('/').unwrap_or(rest).to_string();
                }
            }
        }

        symbol.dependencies.sort();
    }
...
fn compare_structural_properties(
    ...
    for rust_sym in &rust_graph.symbols {
        let ts_sym = match oracle_by_id.get(rust_sym.id.as_str()) {
            Some(s) => *s,
            None => {
                panic!(
        ...
        if let Some(ts_kind_name) = ts_sym.get("kindName").and_then(|k| k.as_str()) {
            assert_eq!(
                rust_sym.kind_name, ts_kind_name,
        ...
        if let Some(ts_file_path) = ts_sym.get("filePath").and_then(|f| f.as_str()) {
            assert_eq!(
                rust_sym.file_path, ts_file_path,
        ...
        if let Some(ts_heritage) = ts_sym.get("heritage").and_then(|h| h.as_array()) {
            let ts_heritage_strs: Vec<&str> = ts_heritage
                ...
                .collect();
            assert_eq!(
                rust_sym.heritage, ts_heritage_strs,
