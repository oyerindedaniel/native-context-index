...
/// Errors that can occur during package scanning.
#[derive(Debug, thiserror::Error)]
pub enum ScanError {
    /// The provided `node_modules` path does not exist.
    #[error("node_modules not found at: {path}")]
    NotFound {
        path: PathBuf,
    },

    /// A filesystem I/O error occurred during scanning.
...
mod tests {
    ...
    fn read_package_info_detects_scoped_packages() {
        ...
        let info = read_package_info(temp_dir.path(), "fallback").unwrap();
        assert_eq!(info.name, "@types/react");
        assert!(info.is_scoped);
    }
    ...
    fn scan_packages_discovers_scoped_packages() {
        ...
        let packages = scan_packages(&node_modules).unwrap();
        assert_eq!(packages.len(), 1);
        assert_eq!(packages[0].name, "@types/react");
