...
use crate::types::{
    Deprecation, ParsedExport, ParsedImport, SymbolKind, TypeReference,
    Visibility,
};
...
pub fn parse_file(file_path: &str) -> Option<ParseResult> {
    let source_text = match fs::read_to_string(file_path) {
        Ok(text) => text,
        Err(e) => {
            eprintln!("[nci-engine] Failed to read {}: {}", file_path, e);
            return None;
        }
    };
...
pub fn parse_file_from_source(_file_path: &str, source_text: &str) -> ParseResult {
    ...
    if !parser_return.errors.is_empty() {
        eprintln!(
            ...
            _file_path,
            parser_return.errors.first().map(|e| e.to_string()).unwrap_or_default()
        );
    }
    ...
    let mut local_decls: HashMap<String, LocalDecl<'_>> = HashMap::new();
    for statement in &program.body {
        match statement {
            Statement::TSInterfaceDeclaration(iface) => {
                local_decls.insert(iface.id.name.to_string(), LocalDecl::Interface(&iface.body));
            }
            Statement::TSTypeAliasDeclaration(alias) => {
                local_decls.insert(alias.id.name.to_string(), LocalDecl::Type(&alias.type_annotation));
            }
            Statement::ClassDeclaration(class) => {
                if let Some(id) = &class.id {
                    local_decls.insert(id.name.to_string(), LocalDecl::Class(class));
                }
            }
                ...
                for decl in &var_decl.declarations {
                    if let BindingPattern::BindingIdentifier(ident) = &decl.id {
                        if let Some(anno) = &decl.type_annotation {
                            local_decls.insert(ident.name.to_string(), LocalDecl::Type(&anno.type_annotation));
                        }
                    }
                }
            }
...
fn extract_triple_slash_directives(source_text: &str) -> (Vec<String>, Vec<String>) {
    ...
    for line in source_text.lines() {
        ...
        if let Some(caps) = path_re.captures(trimmed) {
            ...
            if let Some(types) = caps.get(1) {
                type_references.push(types.as_str().to_string());
            }
        }
    }
            ...
            match specifier {
                ImportDeclarationSpecifier::ImportDefaultSpecifier(default_spec) => {
                    imports.push(ParsedImport {
                        name: default_spec.local.name.to_string(),
                        source: source.clone(),
                        original_name: None,
                        is_default: true,
                        is_namespace: false,
                ...
                ImportDeclarationSpecifier::ImportNamespaceSpecifier(namespace_spec) => {
                    imports.push(ParsedImport {
                        name: namespace_spec.local.name.to_string(),
                        source: source.clone(),
                        original_name: None,
                        is_default: false,
                        is_namespace: true,
                ...
                ImportDeclarationSpecifier::ImportSpecifier(named_spec) => {
                    ...
                    let local_name = named_spec.local.name.to_string();
...
fn extract_export_named<'a>(
    ...
    local_decls: &HashMap<String, LocalDecl<'a>>,
) {
    let source_module = export_named
        .source
        .as_ref()
        .map(|source_lit| source_lit.value.to_string());
    ...
    match &export_default.declaration {
        ...
        ExportDefaultDeclarationKind::ClassDeclaration(class_decl) => {
            if let Some(id) = &class_decl.id {
                ...
                extract_class_members(class_decl, source_text, &class_name, false, &jsdoc, exports, local_decls);
            }
        }
        ExportDefaultDeclarationKind::TSInterfaceDeclaration(iface_decl) => {
            let iface_name = iface_decl.id.name.to_string();
            ...
            extract_interface_members(iface_decl, source_text, &iface_name, false, &jsdoc, exports, local_decls);
        }
        _ => {}
    }
}
...
fn extract_export_all(
    ...
    if let Some(exported_name) = &export_all.exported {
        // export * as ns from '...'
        let namespace_name = module_export_name_to_string(exported_name);
        ...
        exports.push(ParsedExport {
            ...
            signature: Some(format!(
                "{} * as {} from '{}'",
                type_prefix, namespace_name, source
            )),
            ..Default::default()
        });
    } else {
        ...
        exports.push(ParsedExport {
            ...
            is_wildcard: true,
            is_explicit_export: true,
            signature: Some(format!("{} * from '{}'", type_prefix, source)),
            ..Default::default()
        });
    }
...
fn extract_module_declaration<'a>(
    ...
    let module_name = match &module_decl.id {
        ...
        TSModuleDeclarationName::StringLiteral(string_lit) => string_lit.value.to_string(),
    };

    let is_global = module_name == "global";
    ...
    exports.push(ParsedExport {
        ...
        visibility: jsdoc.visibility,
        since: jsdoc.since,
        modifiers: if !is_global && !is_string_module { vec!["export".to_string()] } else { Vec::new() },
        ..Default::default()
    });
                    ...
                    if let Some(declaration) = &export_named.declaration {
                        ...
                        for export_item in &mut decl_exports {
                            if !export_item.name.starts_with(parent_name) {
                                export_item.name =
                                    format!("{}.{}", parent_name, export_item.name);
                            }
                        }
                        exports.extend(decl_exports);
                        continue;
...
fn extract_declaration<'a>(
    ...
    match declaration {
        Declaration::FunctionDeclaration(func_decl) => {
            ...
            let mut export_item = ParsedExport {
                name,
                kind: SymbolKind::Function,
                is_explicit_export,
                signature: Some(signature),
                ...
                visibility: jsdoc.visibility,
                since: jsdoc.since,
                modifiers: extract_declaration_modifiers(declaration),
                ..Default::default()
            };
            if is_explicit_export {
                export_item.modifiers.push("export".to_string());
                export_item.modifiers.sort();
            }
        ...
        Declaration::TSTypeAliasDeclaration(type_alias) => {
            ...
            results.push(ParsedExport {
                ...
                since: jsdoc.since,
                modifiers: {
                    let mut mods = extract_declaration_modifiers(declaration);
                    if is_explicit_export {
            ...
            for declarator in &var_decl.declarations {
                if let BindingPattern::BindingIdentifier(binding_ident) = &declarator.id {
                    ...
                    let mut export_item = ParsedExport {
                        ...
                        visibility: jsdoc.visibility.clone(),
                        since: jsdoc.since.clone(),
                        modifiers: if var_decl.declare { vec!["declare".to_string()] } else { Vec::new() },
                        ..Default::default()
                    };
                    if is_exported {
                        export_item.modifiers.push("export".to_string());
...
fn extract_class_members<'a>(
    ...
    _local_decls: &HashMap<String, LocalDecl<'a>>,
) {
    for member in &class_decl.body.body {
        ...
        let qualified_name = if is_static {
            ...
            format!("{}.prototype.{}", parent_name, member_name)
        };
        ...
        let dependencies = deps_map.into_values().collect();
        ...
        results.push(ParsedExport {
            ...
            modifiers: if is_static { vec!["static".to_string()] } else { Vec::new() },
            ..Default::default()
        });
    }
}
...
fn extract_class_body_as_type_members<'a>(
    ...
    for member in &class_decl.body.body {
        ...
        let qualified_name = format!("{}.{}", parent_name, member_name);
        let signature = get_span_text(source_text, member_span);
        let member_jsdoc = extract_jsdoc_from_leading_comments(source_text, member_span);
    ...
    for member in &iface_decl.body.body {
        let (member_name_opt, member_span, member_kind) = match member {
            ...
            TSSignature::TSCallSignatureDeclaration(call_sig) => {
                (Some("()".to_string()), call_sig.span, SymbolKind::MethodSignature)
            }
            _ => continue,
        };
        ...
        let qualified_name = if member_name == "()" {
    ...
    match ts_type {
        ...
        TSType::TSTypeReference(type_ref) => {
            ...
            if !visited.contains(&ref_name) {
                ...
                if let Some(local_decl) = local_decls.get(&ref_name) {
                    match local_decl {
        ...
        TSType::TSImportType(import_type) => {
            if let Some(qualifier) = &import_type.qualifier {
                if let TSImportTypeQualifier::Identifier(ident) = qualifier {
                    let name = ident.name.to_string();
                    let import_path = Some(import_type.source.value.to_string());
                    if !BUILTIN_TYPES.contains(name.as_str()) {
...
fn extract_heritage_from_class(class_decl: &Class<'_>) -> Vec<String> {
...
fn extract_heritage_from_interface(iface_decl: &TSInterfaceDeclaration<'_>) -> Vec<String> {
    let mut heritage: Vec<String> = Vec::new();
    ...
    match expr {
        ...
        Expression::StaticMemberExpression(member) => {
            if let Some(obj) = expression_to_string(&member.object) {
                return Some(format!("{}.{}", obj, member.property.name));
            }
            None
        }
        _ => None,
...
fn ts_type_name_to_string(type_name: &TSTypeName<'_>) -> String {
    match type_name {
        TSTypeName::IdentifierReference(ident) => ident.name.to_string(),
        TSTypeName::QualifiedName(qual) => {
            format!("{}.{}", ts_type_name_to_string(&qual.left), qual.right.name)
        }
...
fn extract_jsdoc_from_leading_comments(source_text: &str, span: oxc_span::Span) -> JsDocInfo {
    ...
    if let Some(comment_start) = trimmed.rfind("/**") {
            ...
            if cleaned_line.starts_with("@deprecated") {
                if result.deprecated.is_none() {
                    ...
                    result.deprecated = Some(if message.is_empty() {
                        ...
                        Deprecation::Message(message.to_string())
                    });
                }
                ...
                if result.since.is_none() {
                    ...
                    if !version.is_empty() {
                        result.since = Some(version.to_string());
                    }
                }
        ...
        if !doc_lines.is_empty() {
            result.js_doc = Some(doc_lines.join("\n"));
        }
    }
...
fn get_span_text(source_text: &str, span: oxc_span::Span) -> String {
    let start = span.start as usize;
...
fn module_export_name_to_string(name: &ModuleExportName<'_>) -> String {
    match name {
        ...
        ModuleExportName::StringLiteral(string_lit) => string_lit.value.to_string(),
    }
}

fn property_key_to_string(key: &PropertyKey<'_>, source_text: &str) -> Option<String> {
    match key {
        PropertyKey::StaticIdentifier(ident) => Some(ident.name.to_string()),
        PropertyKey::StringLiteral(string_lit) => Some(string_lit.value.to_string()),
        PropertyKey::NumericLiteral(num_lit) => num_lit.raw.as_ref().map(|raw_val| raw_val.to_string()),
        key if key.is_expression() => {
            let expr = key.as_expression().unwrap();
            match expr {
                Expression::StringLiteral(s) => Some(s.value.to_string()),
                Expression::NumericLiteral(n) => n.raw.as_ref().map(|v| v.to_string()).or_else(|| Some(n.value.to_string())),
                _ => {
                    let span_text = get_span_text(source_text, expr.span());
                    if span_text.is_empty() {
                        None
                    } else {
                        Some(format!("[{}]", span_text))
                    }
                }
            }
        }
...
fn extract_declaration_modifiers(declaration: &Declaration<'_>) -> Vec<String> {
    let mut modifiers = Vec::new();

    match declaration {
        Declaration::VariableDeclaration(v) => {
            if v.declare {
                modifiers.push("declare".to_string());
            }
...
mod tests {
    ...
    fn parse_interface_export() {
        ...
        "#;
        let result = parse_file_from_source("test.d.ts", source);
        assert!(result.exports.iter().any(|export_item| export_item.name == "Config" && export_item.kind == SymbolKind::Interface));
    }
    ...
    fn parse_class_with_members() {
        ...
        "#;
        let result = parse_file_from_source("test.d.ts", source);
        assert!(result.exports.iter().any(|export_item| export_item.name == "Logger"));
        assert!(result.exports.iter().any(|export_item| export_item.name == "Logger.prototype.info"));
        assert!(result.exports.iter().any(|export_item| export_item.name == "Logger.create"));
    }
    ...
    fn parse_reexport_wildcard() {
        ...
        assert_eq!(result.exports[0].source, Some("./core".to_string()));
    }
    ...
    fn parse_namespace_reexport() {
        ...
        let result = parse_file_from_source("test.d.ts", source);
        assert_eq!(result.exports.len(), 1);
        assert_eq!(result.exports[0].name, "utils");
    ...
    fn parse_named_reexports() {
        ...
        assert!(result.exports.iter().any(|export_item| export_item.name == "Foo"));
        let baz = result.exports.iter().find(|export_item| export_item.name == "Baz").unwrap();
        assert_eq!(baz.original_name, Some("Bar".to_string()));
    }
    ...
    fn parse_type_alias() {
        ...
        let result = parse_file_from_source("test.d.ts", source);
        assert!(result.exports.iter().any(|export_item| export_item.name == "Result" && export_item.kind == SymbolKind::TypeAlias && export_item.is_type_only));
    }
    ...
    fn parse_enum_declaration() {
        ...
        let result = parse_file_from_source("test.d.ts", source);
        assert!(result.exports.iter().any(|export_item| export_item.name == "Color" && export_item.kind == SymbolKind::Enum));
    }
    ...
    fn parse_variable_declaration() {
        ...
        let result = parse_file_from_source("test.d.ts", source);
        assert!(result.exports.iter().any(|export_item| export_item.name == "VERSION" && export_item.kind == SymbolKind::Variable));
    }
    ...
    fn parse_imports() {
        ...
        assert!(result.imports.iter().any(|import_item| import_item.name == "Foo" && !import_item.is_namespace));
        assert!(result.imports.iter().any(|import_item| import_item.name == "utils" && import_item.is_namespace));
        assert!(result.imports.iter().any(|import_item| import_item.name == "defaultExport" && import_item.is_default));
    }
    ...
    fn parse_jsdoc_deprecated() {
        ...
        "#;
        let result = parse_file_from_source("test.d.ts", source);
        let old_func = result.exports.iter().find(|export_item| export_item.name == "oldFunction").unwrap();
    ...
    fn parse_declare_module() {
        ...
        "#;
        let result = parse_file_from_source("test.d.ts", source);
        assert!(result.exports.iter().any(|export_item| export_item.name == "my-module" && export_item.kind == SymbolKind::Namespace));
    }
    ...
    fn parse_type_references_basic() {
        ...
        let result = parse_file_from_source("test.d.ts", source);
        let func = result.exports.iter().find(|export_item| export_item.name == "process").unwrap();
        let dep_names: Vec<&str> = func.dependencies.iter().map(|dep| dep.name.as_str()).collect();
    ...
    fn parse_type_references_skips_builtins() {
        ...
        let result = parse_file_from_source("test.d.ts", source);
        let func = result.exports.iter().find(|export_item| export_item.name == "foo").unwrap();
