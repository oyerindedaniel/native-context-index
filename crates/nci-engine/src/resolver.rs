...
pub fn resolve_types_entry(package_dir: &Path) -> Result<PackageEntry, ResolveError> {
    ...
    let mut types_entries: Vec<String> = Vec::new();
    let mut subpaths: HashMap<String, String> = HashMap::new();
    ...
    if types_entries.is_empty() {
        if let Some(types_versions) = parsed_pkg.get("typesVersions") {
            if let Some(resolved) = resolve_types_versions(package_dir, types_versions) {
                subpaths.insert(".".to_string(), resolved.clone());
    ...
    if types_entries.is_empty() {
        ...
        if fallback_path.is_file() {
            ...
            subpaths.insert(".".to_string(), normalized.clone());
    ...
    Ok(PackageEntry {
        name,
        dir_path: package_dir.to_path_buf(),
        types_entries,
        subpaths,
    })
}

pub fn resolve_module_specifier(specifier: &str, current_file: &str) -> Vec<String> {
...
fn resolve_all_exports(
    ...
    subpaths: &mut HashMap<String, String>,
) -> Vec<String> {
    ...
    match exports {
        serde_json::Value::String(export_string) => {
            if is_declaration_file(export_string) {
                if let Some(resolved) = resolve_file(package_dir, export_string) {
                    ...
                    subpaths.entry(".".to_string()).or_insert(resolved);
                }
            }
        }
        ...
        serde_json::Value::Object(exports_map) => {
            ...
            if has_subpaths {
                for (subpath_key, subpath_value) in exports_map {
                    if !subpath_key.starts_with('.') {
                        continue;
                    }
                    if subpath_key == "./package.json" {
                        continue;
                    }
                    ...
                    {
                        subpaths.insert(subpath_key.clone(), resolved.clone());
...
fn expand_wildcard_subpath(
    ...
    for candidate_path in &file_candidates {
        ...
        {
            matching_entries.push(candidate_path.to_string_lossy().replace('\\', "/"));
        }
    }
...
fn resolve_relative_specifier(specifier: &str, current_file: &str) -> Vec<String> {
...
fn resolve_package_entry(specifier: &str, current_file: &str) -> Vec<String> {
    ...
    if let Some(mapped_path) = pkg_entry.subpaths.get(&subpath) {
        return vec![mapped_path.clone()];
    }
    ...
    if let Some(resolved) = resolve_file(&pkg_dir, &subpath_normalized) {
        return vec![normalize_path(Path::new(&resolved))];
    }
...
fn resolve_file(package_dir: &Path, relative_path: &str) -> Option<String> {
...
///
/// Per Decision #5: Forward slashes everywhere in output for cross-platform parity.
pub fn normalize_path(file_path: &Path) -> String {
...
mod tests {
    ...
    fn resolve_types_entry_works_without_pkg_json() {
        ...
        assert_eq!(entry.name, temp_dir.path().file_name().unwrap().to_string_lossy());
    }
