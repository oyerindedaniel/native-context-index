...
# Source of truth: repo-root install.sh (copied to apps/web/public/ by sync-install-sh).
#
...
#    (same logic as postinstall; bash alone does not gunzip the release asset).
set -euo pipefail
