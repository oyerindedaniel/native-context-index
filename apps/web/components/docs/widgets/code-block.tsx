...
export function CodeBlockShell({
  ...
  return (
    <pre
      className={cn("nci-panel-stack group/code relative my-6", className)}
      {...rest}
    >
      <button
        type="button"
        ...
        className={cn(
          "absolute right-3 top-3 z-10 inline-flex h-7 w-7 cursor-pointer items-center justify-center rounded-md border border-border/70 bg-elevated/95 text-muted/85 opacity-0 shadow-[0_1px_2px_#0000000a] backdrop-blur-sm transition-[opacity,background-color,color,border-color,transform] duration-150 ease-out hover:border-primary/35 hover:bg-elevated hover:text-ink focus-visible:opacity-100 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/35 focus-visible:ring-offset-2 group-hover/code:opacity-100 active:scale-[0.97]",
        )}
      >
        <StatusIcon
