...
console.log(`   Gap:          ${(tsSymbols.length - rustSymbols.length).toLocaleString()}`);

const getSymbolKey = (symbol: SymbolEntry) => `${path.basename(symbol.filePath)}::${symbol.name}`;

const rustSet = new Set(rustSymbols.map(getSymbolKey));
const missingInRust = tsSymbols.filter(symbol => !rustSet.has(getSymbolKey(symbol)));
...
Object.entries(kindCounts)
    ...
    .forEach(([kind, count]) => {
        ...
        console.log(` - ${kind.padEnd(25)} ${count.toString().padStart(5)} (${percentage}%)`);
    });
...
missingInRust.slice(0, 15).forEach(symbol => {
    console.log(` - [${symbol.kindName}] ${symbol.name} (in ${path.basename(symbol.filePath)})`);
});
